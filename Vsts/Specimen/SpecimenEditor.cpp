...
{
	pressedLoadGsmSample = false;
	pressedLoadBlobSample = false;
...
void SpecimenEditor::Open()
{
	...
	addSpacer();
	addKnob((VstInt32)Specimen::ParamIndices::VoiceMode, "MODE");
	addKnob((VstInt32)Specimen::ParamIndices::SlideTime, "SLIDE");
	addSpacer();
...
bool UpdateButtonReleased(bool *dst, bool newValue)
{
	auto oldValue = *dst;
	*dst = newValue;
...
{
	if (!frame) return;
	if (index == (int)Specimen::ParamIndices::LoadGsmSample && UpdateButtonReleased(&pressedLoadGsmSample, value != 0.0f))
	{
		LoadGsmSample();
	}
	else if (index == (int)Specimen::ParamIndices::LoadBlobSample && UpdateButtonReleased(&pressedLoadBlobSample, value != 0.0f))
	{
		LoadBlobSample();
	}
	else
...
void SpecimenEditor::LoadGsmSample()
{
	VstFileSelect vfs;
...
void SpecimenEditor::LoadBlobSample()
{
	VstFileSelect vfs;
	memset(&vfs, 0, sizeof(vfs));
	vfs.command = kVstFileLoad;
	...
	if (fileSelector->run(&vfs))
	{
		try
		{
			...
			if (!input.is_open()) throw exception("Could not open file.");
			auto inputSize = input.tellg();
			auto inputBuf = new char[(unsigned int)inputSize];
			input.seekg(0, ios::beg);
			input.read((char *)inputBuf, inputSize);
			input.close();

			SampleFormat sampleFormat;
			sampleFormat.Tag = SampleFormat::kTag;
			sampleFormat.Version = SampleFormat::kVersion;
