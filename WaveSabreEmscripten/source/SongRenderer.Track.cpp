...
namespace WaveSabrePlayerLib
{
	SongRenderer::Track::Track(SongRenderer *songRenderer, SongRenderer::DeviceFactory factory)
	{
		...
		if (numAutomations)
		{
			...
			for (int i = 0; i < numAutomations; i++)
			{
				...
				Device* device =  songRenderer->devices[devicesIndicies[deviceIndex]];
				automations[i] = new Automation(songRenderer, device);
			}
		}
	...
	void SongRenderer::Track::Run(int numSamples)
	{
		...
		for (int i = 0; i < numDevices; i++) {
			double songPosition = (double)lastSamplePos / Helpers::CurrentSampleRate;
			auto device = songRenderer->devices[devicesIndicies[i]];
			if (device)
				device->Run(songPosition, Buffers, Buffers, numSamples);
		}
	...
	void SongRenderer::Track::Automation::Run(int numSamples)
	{
		...
		{
			if (device) device->SetParam(paramId, points[numPoints - 1].Value);
		}
		else if (pointIndex <= 0)
		{
			if (device) device->SetParam(paramId, points[0].Value);
		}
		else
		{
			...
			float mixAmount = timestampDelta > 0 ?
				...
				0.0f;
			if (device) device->SetParam(paramId, Helpers::Mix(points[pointIndex - 1].Value, points[pointIndex].Value, mixAmount));
		}
